...
var glslRepresentation = function glslRepresentation(value, typeHint) {
  ...
  if (value instanceof three.Color) return "vec3(".concat(g(value.r), ", ").concat(g(value.g), ", ").concat(g(value.b), ")");
  if (value instanceof three.Vector2) return "vec2(".concat(g(value.x), ", ").concat(g(value.y), ")");
  if (value instanceof three.Vector3) return "vec3(".concat(g(value.x), ", ").concat(g(value.y), ", ").concat(g(value.z), ")");
  if (value instanceof three.Vector4) return "vec4(".concat(g(value.x), ", ").concat(g(value.y), ", ").concat(g(value.z), ", ").concat(g(value.w), ")");
  if (value instanceof three.Matrix3) return "mat3(".concat(value.toArray().map(function (n) {
    return g(n);
  }).join(", "), ")");
  if (value instanceof three.Matrix4) return "mat4(".concat(value.toArray().map(function (n) {
    return g(n);
  }).join(", "), ")");
...
var ScaleAndOffset = function ScaleAndOffset(v) {
  ...
  return Unit(type(v), _function.pipe(v, function (v) {
    return Mul(v, scale);
  }, function (v) {
    return Add(v, offset);
  }), {
...
exports.NormalizePlusMinusOne = NormalizePlusMinusOne;
exports.OneMinus = OneMinus;
...
exports.ScaleAndOffset = ScaleAndOffset;
exports.Sign = Sign;
exports.Sin = Sin;
exports.SingleArgumentFunction = SingleArgumentFunction;
